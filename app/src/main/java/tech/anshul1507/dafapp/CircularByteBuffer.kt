...
open class CircularByteBuffer @JvmOverloads constructor(
        ...
        override fun read(): Int {
            while (true) {
                synchronized(this@CircularByteBuffer) {
                    ...
                    if (available > 0) {
                        val result: Int = (buffer[readPosition] and 0xff.toByte()).toInt()
                        readPosition++
        ...
        override fun write(cbuf: ByteArray, off: Int, len: Int) {
            ...
            while (len > 0) {
                synchronized(this@CircularByteBuffer) {
                    ...
                    if (!blockingWrite && spaceLeft < len) throw BufferOverflowException("CircularByteBuffer is full; cannot write $len bytes")
        ...
        override fun write(c: Int) {
            ...
            while (!written) {
                synchronized(this@CircularByteBuffer) {
                    ...
                    if (!blockingWrite && spaceLeft < 1) throw BufferOverflowException("CircularByteBuffer is full; cannot write 1 byte")
                    if (spaceLeft > 0) {
